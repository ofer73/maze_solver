...
if __name__ == "__main__":
    ...
    while(a != (e[0],e[1])):
        ...
        if not(visited.__contains__(a)):
            visited.append(a)
        if not(visited.__contains__((a[0],a[1]-1)) or edges[a[0]][a[1]-1] == 255 or queue.__contains__((a[0],a[1]-1))): #left or wall
            queue.append((a[0],a[1]-1))
            backTrack[a[0]][a[1]-1] = a
        if not(visited.__contains__((a[0],a[1]+1)) or edges[a[0]][a[1]+1] == 255 or queue.__contains__((a[0],a[1]+1))): #right or wall
            queue.append((a[0],a[1]+1))
            backTrack[a[0]][a[1]+1] = a
        if not(visited.__contains__((a[0]+1,a[1])) or edges[a[0]+1][a[1]] == 255 or queue.__contains__((a[0]+1,a[1]))): #up or wall
            queue.append((a[0]+1,a[1]))
            backTrack[a[0]+1][a[1]] = a
        if not(visited.__contains__((a[0]-1,a[1])) or edges[a[0]-1][a[1]] == 255 or queue.__contains__((a[0]-1,a[1]))): #down or wall
            queue.append((a[0]-1,a[1]))
            backTrack[a[0]-1][a[1]] = a
